...
namespace Unity.Reflect.Actor.Samples
{
    public class AddingCollidersSample : MonoBehaviour
    {
        ...
        void Start()
        {
            // Create the actors and connections
            ...
            // Create and connect the AddCollider actor setup for this sample
            var addColliderSetup = asset.CreateActorSetup<AddColliderActor>();
            var instanceStreamingSetup = asset.GetActorSetup<InstanceStreamingActor>();
            asset.ConnectNet<GameObjectCreated>(instanceStreamingSetup, addColliderSetup);
