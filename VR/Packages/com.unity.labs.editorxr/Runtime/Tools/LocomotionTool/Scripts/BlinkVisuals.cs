...
#pragma warning disable 649
    ...
    [SerializeField]
    [Range(0f, 1f)]
    [Tooltip("From 0 (horizontal & vertical) to 1 (horizontal only).")]
    float m_MaxValidInclination = 0.5f;
...
#pragma warning restore 649
    ...
    void Update()
    {
        ...
        for (var i = 0; i < m_MaxProjectileSteps; i++)
        {
            ...
            else
            {
                ...
                if (this.Raycast(ray, out hit, out go, segment.magnitude + scaledEpsilon, ignoreList))
                {
                    ...
                    isValidInclination &= Vector3.Dot(Vector3.up, hit.normal) >= m_MaxValidInclination;
                }
        ...
        if (targetPosition.HasValue && isValidInclination)
        {
        ...
        if (!m_Visible || !isValidInclination || Vector3.Dot(transform.forward, gravity.normalized) < m_InvalidThreshold)
            targetPosition = null;
