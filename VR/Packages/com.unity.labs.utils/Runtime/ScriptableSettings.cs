...
namespace Unity.Labs.Utils
{
    ...
    {
        const string k_SavePathFormat = "Assets/{0}Resources/ScriptableSettings/{1}.asset";
        ...
        public static T CreateAndLoad()
        {
            ...
            s_Instance = Resources.Load(string.Format(k_LoadPathFormat, GetFilePath())) as T;
...
namespace Unity.Labs.Utils.Internal
{
    ...
    public abstract class ScriptableSettingsBase<T> : ScriptableSettingsBase where T : ScriptableObject
    {
        ...
        protected static void Save(string savePathFormat)
        {
            ...
            if (type.IsDefined(typeof(ScriptableSettingsPathAttribute), true))
            {
                var pathAttribute = s_Instance.GetType().GetAttribute<ScriptableSettingsPathAttribute>(true);
                string path;
                ...
                if (generatePath)
                    Debug.LogWarningFormat("The path '{0}' is invalid. Generating a path instead.", pathAttribute.path);
            }
            ...
            if (generatePath)
            {
                ...
                var specializationPath = folderEnd > 0 ? string.Concat(assetPath.Substring(folderStart, folderEnd), "/") : "";
